...
class Day10 implements SolutionInterface
{
    ...
    public function part1(string $input, bool $visualize = false): int
    {
        ...
        for ($x = 0; $x < $size; $x++) {
            for ($y = 0; $y < $size; $y++) {
                if ($grid[$y][$x] === 0) {
                    $found = [];
                    $answer += $this->traverse($grid, $x, $y, $size, 0, $found);
                }
            }
        }
    ...
    private function traverse(array $grid, int $x, int $y, int $size, int $currentValue, array &$found): int
    {
        if ($currentValue === 9) {
            if (isset($found[$y][$x])) {
                return 0;
        ...
        if ($x > 0 && $grid[$y][$x - 1] === $nextVal) {
            $total += $this->traverse($grid, $x - 1, $y, $size, $nextVal, $found);
        }
        ...
        if ($y > 0 && $grid[$y - 1][$x] === $nextVal) {
            $total += $this->traverse($grid, $x, $y - 1, $size, $nextVal, $found);
        }
